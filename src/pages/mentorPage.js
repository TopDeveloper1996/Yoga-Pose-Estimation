...
function MentorPage() {
  ...
  const addCard = () => {
    let newCardList = cardInfo[0];
    setCards((cardInfo) => [...cardInfo, newCardList]);
  };
  const removeCard = (id) => {
    setCards((cardInfo) => cardInfo.filter((_, index) => index !== id));
  };
  useEffect(() => {});
            ...
            >
              {cardInfo.map((card, index) => (
                <PoseCard
                  ...
                  id={cardInfo.length - index}
                  card={cardInfo[cardInfo.length - index - 1]}
                />
              ))}
            </div>
            <button className="mx-1 w-full md:h-10 bg-[#004392]   shadow-gray-500  shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded text-center text-white text-lg content-center">
              Save
            </button>
          </div>
        </div>
