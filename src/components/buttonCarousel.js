...
const ButtonCarousel = () => {
  ...
  const handleRightClick = () => {
    setStartIndex((prevIndex) =>
      Math.min(prevIndex + 1, buttonLabels.length - itemsToShow)
    );
  };
  ...
  return (
    <div className="flex justify-between w-full h-9">
      ...
      <button
        ...
        disabled={startIndex + itemsToShow >= buttonLabels.length} // Disable if at end
        className={`mr-3 text-white font-bold arrow-button ${
          startIndex + itemsToShow >= buttonLabels.length ? "disabled" : ""
        }`}
      >
        &gt; {/* Right arrow */}
      </button>
