  ...
  return (
    <div className="absolute x-80 y-10 top-0 right-0 inline-flex bg-transparent items-center mt-2 mr-3">
      ...
      >
        ...
        <span className="flex float-right ml-1 w-2 text-[#004392]">{level}</span>
      </div>
        <div
        ...
        <span>Match:</span>
        {90 <= Number(percent) && Number(percent) <= 100 ? <span className="text-green-600">{percent}</span>:(Number(percent) < 50 ? <span className="text-red-500">{percent}</span>:<span className="text-gray-600">{percent}</span>)}
        <span>%</span>
      </div>
    </div>
  );
